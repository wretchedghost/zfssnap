...
# The number of snapshots is defined in the variable retention.
...
# Full paths to these utilities are needed when running the script from cron. Certain utility file paths are for Linux only.
date=/bin/date
grep=/usr/bin/grep
sed=/usr/bin/sed
sort=/usr/bin/sort
